...
WHERE `tests`.`test_id` = `test_questions`.`test_id` 
  ...
  AND  `test_questions`.`test_id` ='" . $test_id . "'";
	...
	public function print_dmc($exam_id, $student_id)
	{
		...
		$this->data['exam_id'] = $exam_id = (int) $exam_id;
		$this->data['student_id'] = $student_id = (int) $student_id;
		$query = "SELECT * FROM exams WHERE exam_id ='" . $exam_id . "'";
	...
	public function print_combine_dmc($exam_id, $student_id)
	{
	...
	public function print_class_combine_dmcs($exam_id, $class_id, $section_id)
	{
		...
		AND sm.exam_id = '" . $exam_id . "' 
		GROUP BY sm.student_id ORDER BY s.student_class_no ASC";
			...
			if (preg_match('/^(\d+)/', $filename, $matches)) {
				...
				if ($student) {
					...
					if (rename($file, $new_file_path)) {
						$this->db->where('student_id', $student->student_id)->update('students', ['student_image' =>  'student_' . $new_file_name]);
	...
	{
		$this->data['student_id'] = $student_id = (int) $this->input->post('student_id');
		$this->data['exam_id'] = $exam_id = (int) $this->input->post('exam_id');
