...
class Timetable extends Admin_Controller
{
	...
	public function index()
	{
		...
		foreach ($classes as $class) {
			...
			ORDER BY Order By `class_subjects`.`total_class_week`, `subjects`.`order`, `subjects`.`subject_title`, `subjects`.`group`  ASC ";
			$result = $this->db->query($query);
			$class->subjects = $result->result();
		}
			...
			if ($teacher_class->assinged == 0) {
				if (!in_array($teacher_class->class_section_id, $assigned_sections_subjects)) {
					...
					if ($teacher_class->Class_title == '9th' and $teacher_class->subject_title ==  'Library' and $teacher_class->total_class_week == 1) {
						$extra = abs($teacher_class->total_class_week + $count_assigned_class_days - 6);
					}
