...
public function section_dashboard($exam_id, $class_id, $section_id){
	...
	/*	$query="SELECT * FROM `exams` WHERE `exams`.`exam_id` = $exam_id";
		...
		foreach($result_percentages as $result_percentage){
			if($result_percentage->percentage>=0 and $result_percentage->percentage<=10){ $percentage_count["0-10"]+=1; }
			if($result_percentage->percentage>=11 and $result_percentage->percentage<=20){ $percentage_count["11-20"]+=1; }
			if($result_percentage->percentage>=21 and $result_percentage->percentage<=30){ $percentage_count["21-30"]+=1; }
			if($result_percentage->percentage>=30and $result_percentage->percentage<=40){ $percentage_count["31-40"]+=1; }
			if($result_percentage->percentage>=41 and $result_percentage->percentage<=50){ $percentage_count["41-50"]+=1; }
			if($result_percentage->percentage>=51 and $result_percentage->percentage<=60){ $percentage_count["51-60"]+=1; }
			if($result_percentage->percentage>=61 and $result_percentage->percentage<=70){ $percentage_count["61-70"]+=1; }
			if($result_percentage->percentage>=71 and $result_percentage->percentage<=80){ $percentage_count["71-80"]+=1; }
			if($result_percentage->percentage>=81 and $result_percentage->percentage<=90){ $percentage_count["81-90"]+=1; }
			if($result_percentage->percentage>=91 and $result_percentage->percentage<=100){ $percentage_count["91-100"]+=1; }
			}
