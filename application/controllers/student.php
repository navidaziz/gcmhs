...
class Student extends Public_Controller{
	...
	public function class_and_section(){
		...
		*/
		...
		foreach($classes as $classe){
		 ...
		 $this->data["view"] = PUBLIC_DIR."student/student_login_page";
...
public function qustion_answer(){
	...
	function edit_students($class_id, $section_id){
		
		
		
		$this->data['class_id']  = $class_id = (int) $class_id;
		$this->data['section_id']  = $section_id = (int) $section_id;
		$where = "`students`.`status` IN (1,0) and `students`.`class_id`='".$class_id."'
		...
		$this->data["pagination"] = "";
		 $this->data["title"] = "Students";
		//var_dump($sections);
		$this->data["view"] = PUBLIC_DIR."student/students_edit";
        $this->load->view(PUBLIC_DIR."layout", $this->data);
