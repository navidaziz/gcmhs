          ...
          <div class="row"> 
            ...
            <script>
              function update_per_week_classes(class_subject_id){
				...
				data:{ total_class_week: total_class_week }}).done(function( msg ) {  
											//$("#status-"+id).html(msg) 
											//alert(msg);
											});
               ...
               <?php 
				   $query="SELECT * FROM `teachers` Order By `teachers`.`teacher_name` ASC";
              ...
              <div id="error"></div>
                ...
                <tbody>
                  ...
                  <th>Periods</th>
                    ...
                    <td style="background-color:#FF0; color:#000; width:300px !important"><strong>
                    <?php echo $period->period_title; ?></strong>
                    </td>
                    <?php endforeach; ?>
            ...
            `class_subjects`,
            `subjects`   
            WHERE `periods`.`period_id` = `period_subjects`.`period_id`
            AND `teachers`.`teacher_id` = `period_subjects`.`teacher_id`
            AND `class_section_subject_teachers`.`class_section_subject_teacher_id` = `period_subjects`.`class_section_subject_teacher_id`
            ...
            AND `class_section_subject_teachers`.`section_id` =$section->section_id
            AND `class_section_subject_teachers`.`class_id` =$class->class_id
            AND `period_subjects`.`period_id`=$period->period_id
            AND `period_subjects`.`$week`=1";
           
            $result = $this->db->query($query);	
            $already_assinged_classes = $result->result();
            if($already_assinged_classes){
              echo "<td style='text-align:center'>";
            foreach($already_assinged_classes as $already_assinged_class){  
           
            echo "<span style='border:1px solid gray; border-radius:5px; margin:1px; padding:1px; display:block'>
            <small>
            ".@$already_assinged_class->short_title."</small></span>";
            echo "<b>".@$already_assinged_class->teacher_name."</b>";
            }
            echo "</td>";
          }else{
            echo "<td></td>";
