...
<div class="row"> 
  ...
  <div class="col-md-12">
    ...
    <button onclick="convert_numbers()">Convert</button>
	  ...
	  function update_student_subjec_marks(student_exam_subject_mark_id){
		   var subject_mark = $('#marks_'+student_exam_subject_mark_id).val();
		  
		   $.ajax({ 
				type: "POST",
				url: "<?php echo site_url(ADMIN_DIR."students_exams_subjects_marks/update_student_subjec_marks"); ?>/"+student_exam_subject_mark_id,
				data:{ subject_mark: subject_mark }}).done(function( msg ) {  
											//$("#status-"+id).html(msg) 
											//alert(msg);
											});
		  }
      ...
      function update_student_info(student_id){
        ...
        <input type="hidden" name="class_subject_id" value="<?php echo $subject_id ?>" />
          ...
          <table class="table table-bordered">
           ...
           <!-- <th>Update subject marks</th>-->
            ...
            <tbody>
              ...
              <td>
              <a class="btn btn-danger btn-sm" onclick="return confirm('are you sure? may remove student over data?')" href="<?php echo site_url(ADMIN_DIR."students/remove_student/$exam_id/$class_id/$section_id/$class_subject_id/$subject_id/$student->student_id") ?>" >Remove student</a> <a onclick="return confirm('are you sure? may hide student over data?')" class="btn btn-primary btn-sm"  href="<?php echo site_url(ADMIN_DIR."students/dormant_student/$exam_id/$class_id/$section_id/$class_subject_id/$subject_id/$student->student_id") ?>" >Dormant</a>
              </td>
                <td>
                <input id="student_class_no_<?php echo $student->student_id; ?>" type="text" name="student_class_no" value="<?php echo $student->student_class_no; ?>" /></td>
              ...
              <!--  <td><input onkeyup="update_student_info('<?php echo $student->student_id; ?>')" type="button" name="Update Student Info" value="Update Student Info" onclick="update_student_info('<?php echo $student->student_id; ?>')" /></td>-->
                
                <td><input  tabindex="<?php echo $count; ?>" onkeyup="update_student_subjec_marks('<?php echo $student->student_exam_subject_mark_id; ?>')" id="marks_<?php echo $student->student_exam_subject_mark_id; ?>" type="text" name="student_marks[<?php echo $student->student_id; ?>][marks]" value="<?php echo $student->obtain_mark; ?>" /> </td>
                <!--<td><input type="button" name="Update Student subject Mark" value="Update Student subject Mark" onclick="update_student_subjec_marks('<?php echo $student->student_exam_subject_mark_id; ?>')" /></td>-->
                
              <td><a href="<?php echo site_url(ADMIN_DIR."exam_list/remove_duplicate_record/$exam_id/$class_id/$section_id/$class_subject_id/$subject_id/"); ?>/<?php echo $student->student_exam_subject_mark_id; ?>" >Remove duplicate record</a></td>
              </tr>
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><input onkeyup="update_student_info('<?php echo $student->student_id; ?>')" id="student_name_<?php echo $student->student_id; ?>" type="text" name="student_name" value="<?php echo $student->student_name; ?>" /></td>
                <td><input tabindex="<?php echo $count; ?>" type="text"   name="student_marks[<?php echo $student->student_id; ?>][marks]" value="0" /></td>
              </tr>
              <?php 
			  $count++;
