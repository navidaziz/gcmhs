           ...
           <span style="float:right !important; text-align:center"> Principal<br />GCMHS Boy Chitral </span> 
            ...
            <tbody>
			 ...
			 $class_number =1;
			  ...
			  $remarks = "";
			  if($result->section=='Jugoor'){ $remarks =  remarks($percentage, 19.5); }else{
					if($result->section=='Khurkashan Deh'){ $remarks = remarks($percentage, 19.5); }else{
						if($result->class=='5th'){
							$remarks = remarks($percentage, 24.5);
							}else{
								if($result->class=='8th'){
								$remarks = remarks($percentage, 19.5);
								}else{
								$remarks = remarks($percentage, 19.5);
									}
						}
			...
			$pass_fail = pass_fail($percentage); 		
			 if($remarks=='pass' or $remarks=='promote' or 1==1){
				$promoted_class = "";
